...
# docker build -t directed_art .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name directed_art directed_art
...
# Install Node.js (needed for builds; kept in runtime for SSR) and pnpm via corepack
...
ARG SSR_ENABLED
RUN if [ "$SSR_ENABLED" = "true" ]; then pnpm exec vite build --ssr; fi && \
    rm -rf node_modules
